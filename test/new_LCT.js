...
//const slotTest = require("./slot_test")
...
contract("LCT合約", async (accounts) => {
    ...
    before(async () => {
        ...
        chips.sendChips(accounts[1], 700000e8)
        chips.sendChips(accounts[2], 700000e8)
        chips.sendChips(accounts[3], 700000e8)
        chips.sendChips(accounts[4], 700000e8)
        chips.sendChips(accounts[5], 700000e8)

        chips.sendChips(LCT.address, 500000e8);
        chips.sendChips(slot.address, 500000e8);
        chips.sendChips(fruit.address, 500000e8);
        chips.sendChips(jack.address, 500000e8);
    ...
    async function inqPrice() {
        console.log("\n////////買價////////\n")

        let price1 = await LCT.buyPrice.call(1);
        console.log("招財貓價格為:"+price1.toString()+"CHIPS");

        let price2 = await LCT.buyPrice.call(2);
        console.log("招財貓價格為:"+price2.toString()+"CHIPS");

        let price3 = await LCT.buyPrice.call(3);
        console.log("招財貓價格為:"+price3.toString()+"CHIPS");

        console.log("\n////////賣價////////\n")

        let price4 = await LCT.sellPrice.call(1);
        console.log("招財貓價格為:"+price4.toString()+"CHIPS");

        let price5 = await LCT.sellPrice.call(2);
        console.log("招財貓價格為:"+price5.toString()+"CHIPS");

        let price6 = await LCT.sellPrice.call(3);
        console.log("招財貓價格為:"+price6.toString()+"CHIPS");
    }
    ...
    //     let r = await jack.startGame(101e8,{from:accounts[4]});
    ...
    it("查詢擁有招財貓", async () => {
        await inq(accounts[1])
        await inq(accounts[2])
        await inq(accounts[3])
    })
    ...
    // it("領取每日獎勵", async() => {
    //     console.log(await chips.isPrime.call(accounts[1]))
    //     await chips.getAward({from:accounts[1]})
    // })
    ...
    // it("vip測試", async() => {
    //     await chips.bePrime(accounts[3])
    // })

    // it("領取每日獎勵", async() => {
    //     console.log(await chips.isPrime.call(accounts[3]))
    //     await chips.getAward({from:accounts[3]})
    // })
